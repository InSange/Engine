...
namespace gui
{
	...
	void ImguiEditor::Initialize()
	{
		...
		NuNu::graphics::GraphicDevice_DX12* device = NuNu::graphics::GetDevice();
	...
	void ImguiEditor::OnEvent(NuNu::Event& e)
	{
		// Block event when ImGui is capturing mouse or keyboard
		if (mBlockEvent)
		{
			ImGuiIO& io = ImGui::GetIO();
			e.Handled |= e.IsInCategory(NuNu::eEventCategory::EventCategoryMouse) & io.WantCaptureMouse;
			e.Handled |= e.IsInCategory(NuNu::eEventCategory::EventCategoryKeyboard) & io.WantCaptureKeyboard;
		}
	}
	...
	void ImguiEditor::End()
	{
		...
		// Rendering
		ImGui::Render();
		NuNu::graphics::GraphicDevice_DX12* graphicDevice = NuNu::graphics::GetDevice();
		NuNu::graphics::FrameContext* frameCtx = graphicDevice->WaitForNextFrameResources();
